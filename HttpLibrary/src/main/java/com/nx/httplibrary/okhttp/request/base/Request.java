 ...
 * See the License for the specific language managerverning permissions and
...
import com.nx.httplibrary.okhttp.adapter.AdapterParam;
import com.nx.httplibrary.okhttp.adapter.CacheCall;
import com.nx.httplibrary.okhttp.adapter.Call;
import com.nx.httplibrary.okhttp.adapter.CallAdapter;
...
public abstract class Request<T, R extends Request> implements Serializable {
    ...
    public Request(String url) {
        this.url = url;
        baseUrl = url;
        NXHttpManager nxHttpManager = NXHttpManager.getInstance();
        ...
        //添加公共请求参数
        if (nxHttpManager.getCommonParams() != null) params(nxHttpManager.getCommonParams());
        if (nxHttpManager.getCommonHeaders() != null) headers(nxHttpManager.getCommonHeaders());
        //添加缓存模式
        retryCount = nxHttpManager.getRetryCount();
        cacheMode = nxHttpManager.getCacheMode();
        cacheTime = nxHttpManager.getCacheTime();
    }
    ...
    public okhttp3.Call getRawCall() {
        ...
        if (client == null) client = NXHttpManager.getInstance().getOkHttpClient();
